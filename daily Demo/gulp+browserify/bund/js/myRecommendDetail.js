...
(function e(t, n, r) {
  function s(o, u) {
      ...
      window.onload = function () {
        ...
        if (openId) {
          ...
          comom.request(resPath.getResUrl() + '/zs/customer/queryCustLoanDetail/V2_0', data, function (res) {
            ...
            if (res.respCode == 1) {
              //推荐信息
              comom.slt('#user_info').innerHTML = '<div class="list-head">推荐信息</div>' + '<li>申请时间<span class="fr">' + (res.respData.createDateStr || '') + '</span></li>' + '<li>姓名<span class="fr">' + (res.respData.name || '') + '</span></li>' + '<li>手机号<span class="fr">' + (res.respData.cellphone || '') + '</span></li>' + '<li>车辆名称<span class="fr">' + (res.respData.vmodel || '') + '</span></li>';
              //佣金信息
              var sCommissionStr = '';
              if (res.respData.prePkgAmountStr != '-') {
                if (res.respData.actPkgAmountStr != '-') {
                  sCommissionStr = '推荐佣金(元)<span class="fr">' + (res.respData.actPkgAmountStr || '') + '</span>' + '<p class="text-right hidden">预计可获得 ' + (res.respData.prePkgAmountStr || '') + '元，请等待业务完成后确认</p>';
                } else {
                  sCommissionStr = '推荐佣金(元)<span class="fr">还没计算出来哦</span>' + '<p class="text-right f-grey mt10">预计可获得 ' + (res.respData.prePkgAmountStr || '') + '元，请等待业务完成后确认</p>';
                }
              } else {
                sCommissionStr = '推荐佣金(元)<span class="fr f-grey">还没计算出来哦</span>' + '<p class="text-right f-grey mt10">预计可获得 ' + (res.respData.prePkgAmountStr || '') + '元，请等待业务完成后确认</p>';
              }
    ...
    }).call(this, require("XJF/FV"), typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {}, require("buffer").Buffer, arguments[3], arguments[4], arguments[5], arguments[6], "/myRecommendDetail.js", "/");
