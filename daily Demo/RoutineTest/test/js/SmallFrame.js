...
/*
 * ajax.getJson()
 * ajax.getJson('person1.json','aa',function(res){
		console.log(res);
	});
 */
function ajax(){}
	ajax.prototype.getJson = function(url,data,success,error){
		var ajaxData = {
			url: url || "",
			data: data || 'data',
		...
		//发送请求
		xmlhttp.open("GET",ajaxData.url,true);
		...
		xmlhttp.onreadystatechange = function(){
			if(xmlhttp.readyState == 4){
				if(xmlhttp.status == 200){
					ajaxData.success(JSON.parse(xmlhttp.response));		
				}else{
					ajaxData.error();
				}
			}
		...
		run:function(){
			...
			if(aList.length <= 0){
				...
				setInterval(function(){
						//判断是否需要过度效果	
					if(bFade){
						aList[0].style.transition = 'margin-top 0.3s';							
					}
						//判断轮播的方向
					if(sDirection.toString()=='up'){							
						...
						nPage++;
						if(nPage >= aList.length){
							nPage = 0;
						}
						if(nPage==1){
							aList[0].style.transition = 'none';								
						}
					}
				//与上相仿
					...
					var nWidth = aList[0].offsetWidth;
						if(bFade){
							aList[0].style.transition = 'margin-left 0.3s';							
						}	
 ...
 * new fBackTop(id);
 */
function fBackTop(el){
	var oRetru = document.getElementById(el);
