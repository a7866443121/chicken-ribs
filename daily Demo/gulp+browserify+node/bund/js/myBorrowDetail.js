...
(function e(t, n, r) {
  function s(o, u) {
    ...
    (function (process, global, Buffer, __argument0, __argument1, __argument2, __argument3, __filename, __dirname) {
      module.exports = {
        ...
        //返回公共参数
        publicData: function publicData() {
        ...
        //地址栏 参数跳转
        forLoadHref: function forLoadHref(jStr) {
          var jsonStr = '';
        ...
        //缺省页创建
        createDefault: function createDefault(obj, src, ms, goTo, btnContent, icofont) {
          var innerSTR;
          if (icofont) {
            innerSTR = '<div class="default-msg"><i class="half"></i><div class="default-box">' + '<i class="iconfont f-theme">' + (icofont || ' ') + '</i><p class="f-666">' + ms + '</p>' + '<p class="f-blue" id="' + (goTo || ' ') + '">' + (btnContent || ' ') + '<i class="icon iconfont f-15">&#xe676;</i></p>' + '</div></div>';
          } else if (goTo) {
            innerSTR = '<div class="default-msg"><i class="half"></i><div class="default-box">' + '<img src="' + src + '" /><p class="f-666">' + ms + '</p>' + '<p class="f-blue" id="' + (goTo || ' ') + '">' + (btnContent || ' ') + '<i class="icon iconfont f-15">&#xe676;</i></p>' + '</div></div>';
          } else {
            innerSTR = '<div class="default-msg"><i class="half"></i><div class="default-box">' + '<img src="' + src + '" /><p class="fcA3">' + ms + '</p>' + '</div></div>';
          }
          obj.innerHTML = innerSTR;
        },
        //缺省页创建按钮
        createDefaultBtn: function createDefaultBtn(obj, src, ms, context, goTo, btnContent, icofont) {
          var innerSTR;
          if (icofont) {
            innerSTR = '<div class="default-msg"><i class="half"></i><div class="default-box">' + '<i class="iconfont f-theme">' + (icofont || ' ') + '</i><p class="f-theme">' + ms + '</p><p class="f-666 mt10">' + context + '</p>' + '<p class="f-blue-btn" id="' + (goTo || ' ') + '">' + (btnContent || ' ') + '</p>' + '</div></div>';
          } else if (goTo) {
            innerSTR = '<div class="default-msg blue-btn"><i class="half"></i><div class="default-box">' + '<img src="' + src + '" /><p class="f-theme">' + ms + '</p><p class="f-666 mt10">' + context + '</p>' + '<p class="f-blue-btn" id="' + (goTo || ' ') + '">' + (btnContent || ' ') + '</p>' + '</div></div>';
          } else {
            innerSTR = '<div class="default-msg"><i class="half"></i><div class="default-box">' + '<img src="' + src + '" /><p class="f-666">' + ms + '</p>' + '</div></div>';
          }
          obj.innerHTML = innerSTR;
        },
        //请求封装
        request: function request(url, data, callBack, erroCallBack, Type, contentType) {
          var ajaxData = {
            type: "post",
            url: url || "",
            async: "true",
        ...
        requestFile: function requestFile(url, data, callBack, erroCallBack, Type, contentType) {
          var xhr = new XMLHttpRequest();
        ...
        //当前节点渐变显示
        hideShow: function hideShow(obj) {
        ...
        //tost
        tost: function tost(content, times) {
          //获取页面原有的tost
    ...
    }).call(this, require("XJF/FV"), typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {}, require("buffer").Buffer, arguments[3], arguments[4], arguments[5], arguments[6], "/..\\..\\api\\comom.js", "/..\\..\\api");
  }, { "XJF/FV": 5, "buffer": 4 }], 2: [function (require, module, exports) {
    (function (process, global, Buffer, __argument0, __argument1, __argument2, __argument3, __filename, __dirname) {
      module.exports = { "path": "http://192.168.1.94:3000" };
    }).call(this, require("XJF/FV"), typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {}, require("buffer").Buffer, arguments[3], arguments[4], arguments[5], arguments[6], "/..\\..\\api\\respPath.js", "/..\\..\\api");
  ...
  }, { "XJF/FV": 5, "buffer": 4 }], 7: [function (require, module, exports) {
    (function (process, global, Buffer, __argument0, __argument1, __argument2, __argument3, __filename, __dirname) {
      //引入comom
        ...
        if (openId) {
          ...
          comom.request(resPath.getResUrl() + '/zs/customer/queryCustLoanDetail/V2_0', data, function (res) {
            if (res.respCode == 1) {
              ...
              comom.slt('#header').innerHTML = '<i class="icon iconfont">&#xe604;</i>' + '<p class="f-14">' + (res.respData.orderState || '目前没有状态') + '</p>' + recordStr;

              //意向信息
              comom.slt('#user_info').innerHTML = '<div class="list-head">意向信息</div>' + '<li>申请时间<span class="fr">' + (res.respData.createDateStr || '-') + '</span></li>' + '<li>姓名<span class="fr">' + (res.respData.name || '-') + '</span></li>' + '<li>手机号<span class="fr">' + (res.respData.cellphone || '-') + '</span></li>';
              //是否有车辆信息?渲染:为空;
              if (res.respData.vmodel) {
                comom.slt('#car_info').innerHTML = '<li>车辆名称<span class="fr">' + (res.respData.vmodel || '-') + '</span></li>' + '<li>首次上牌时间<span class="fr">' + (res.respData.firstcardDate || '-') + '</span></li>' + '<li>行驶里程(万公里)<span class="fr">' + (res.respData.mileage || '-') + '万公里</span></li>' + '<li>意向贷款金额(元)<span class="fr">' + (res.respData.loanPriceStr || '-') + '</span></li>' + '<li>意向贷款期限(期)<span class="fr">' + (res.respData.loanLimit || '-') + '</span></li>';
              } else {
                comom.slt('#car_info').outerHTML = '';
              }
              //业务信息
              comom.slt('#business_info').innerHTML = '<div class="list-head">业务信息</div>' + '<li>订单流水号<span class="fr">' + (res.respData.orderNo || '-') + '</span></li>' + '<li>业务员<span class="fr">' + (res.respData.apply || '-') + (res.respData.applyPhone ? '(' + res.respData.applyPhone + ')' : '') + '</span></li>' + '<li>实际放款金额(元)<span class="fr">' + (res.respData.actLoanAmountStr || '-') + '</span></li>' + '<li>贷款期数(期)<span class="fr">' + (res.respData.actLoanLimit || '-') + '</span></li>' + '<li>执行利率(%)<span class="fr">' + (res.respData.actRate || '-') + '</span></li>' + '<li>还款计划<span class="fr f-grey" id="toView">' + (res.respData.repaymentPlanList && res.respData.repaymentPlanList.length > 0 ? '查看' : '暂未生成') + '<i class="icon iconfont">&#xe676;</i></span></li>';
              //审批记录弹窗 	
              //是否存在这个数组.
    ...
    }).call(this, require("XJF/FV"), typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {}, require("buffer").Buffer, arguments[3], arguments[4], arguments[5], arguments[6], "/myBorrowDetail.js", "/");
  }, { "../../api/comom.js": 1, "../../api/respPath.js": 2, "XJF/FV": 5, "buffer": 4 }] }, {}, [7]);